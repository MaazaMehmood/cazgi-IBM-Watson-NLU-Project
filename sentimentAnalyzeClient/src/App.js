...
class App extends React.Component {
  ...
  */
  ...
  If the requested input mode is "url" it returns a textbox with 3 row.
  */
  ...
  renderOutput = (input_mode)=>{
    ...
    if(input_mode === "text"){
      ...
      this.setState({innercomp:<textarea rows={rows} cols="50" id="textinput" placeholder='Enter text or URL here'/>,
      mode: mode,
      sentimentOutput:[],
      sentiment:true
      });
  ...
  sendForSentimentAnalysis = () => {
    ...
    fetch(url).then((response)=>{
        ...
        output = <div style={{color:color,fontSize:20,fontWeight:'bold',margin:'1rem',padding:'1rem'}}>{output}</div>
